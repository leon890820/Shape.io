class Game {
  ...
  void liteSetting(){
    ...
    if(!liteMap) {
      liteLerp=lerp(liteLerp,0,0.1);      
    }
    else{
      Vector2 position=cam.transform.getPosition();
      liteLerp=lerp(liteLerp,255,0.1);
      rectMode(CENTER);
      fill(192,liteLerp);
      rect(width/2,height/2,width,height);           
      fill(220,liteLerp);      
      rect(0-position.x+width/2,0-position.y+height/2,scl*30,scl*30);
    }
  }
